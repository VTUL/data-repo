...
module GenericFileHelper
  ...
  def render_download_icon(title = nil)
    if title.nil?
      link_to download_image_tag, sufia.download_path(@generic_file), download: (@generic_file.filename || ""), title: "Download the document", id: "file_download", data: { label: @generic_file.id }
    else
      label = download_image_tag(title)
      link_to label, sufia.download_path(@generic_file), download: (@generic_file.filename || ""), title: title, id: "file_download", data: { label: @generic_file.id }
    end
  end

  def render_download_link(text = nil)
    label = text || 'Download'
    link_to label, sufia.download_path(@generic_file), id: "file_download", download: (@generic_file.filename || ""), data: { label: @generic_file.id }
  end
