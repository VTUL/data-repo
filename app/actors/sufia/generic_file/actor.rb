  ...
  class Actor
    ...
    class << self
      def virus_check(file)
        file_obj = File.open(file.to_s)
        path = file.is_a?(String) ? file : file.path

        # Only run virus check on files 200mb or smaller
        unless file_obj.size <= (200 * 1024 * 1024)
          ActiveFedora::Base.logger.warn "File size exceeds virus checking limit, #{path} not checked"
          return
        end
